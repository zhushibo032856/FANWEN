...
@interface ZZEditCollectionVIew ()
...
@implementation ZZEditCollectionVIew
@dynamic delegate;
@dynamic dataSource;
...
- (instancetype)initWithFrame:(CGRect)frame collectionViewLayout:(nonnull UICollectionViewLayout *)layout
{
    ...
    if (self) {
        ...
        [self addGesture];
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(handelCellClick:) name:KCellStateChange object:nil];
...
- (void)gestureEndOrCancle:(UILongPressGestureRecognizer *)longPressGesture{
    ...
    [UIView animateWithDuration:0.25 animations:^{
        self->_tempMoveCell.center = self.dragCell.center;
...
- (void)handelCellClick:(NSNotification *)notification {
    ...
    ZZEditCell *cell = notification.object;
    ...
    if (indexPath.section == 0) {
        ...
        NSLog(@"%ld",manager.headArray.count);
        if (manager.headArray.count >= 5) {
            [MBProgressHUD showError:@"我的应用区域最多设置5个应用 超出部分请先移除再添加"];
            [self reloadData];
            return;
        }else{
        ...
        for (ZZEditCellModel *model in manager.headArray) {
            NSLog(@"%@",model.title);
        }
...
- (void)setBeginEditing:(BOOL)beginEditing {
    ...
    if (beginEditing) {
        [[NSNotificationCenter defaultCenter] postNotificationName:KCellBeganEditing object:@"yes"];
    }else {
        [[NSNotificationCenter defaultCenter] postNotificationName:KCellBeganEditing object:@"no"];
    }
}
