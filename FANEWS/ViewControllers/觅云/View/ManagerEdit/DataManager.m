    ...
    if (self) {
        ...
        for (int i = 0; i < array.count; i ++) {
            ...
            for (int j = 0; j < listArray.count; j ++) {
                ZZEditCellModel *model = [ZZEditCellModel new];
        ...
        for (int i = 0; i < 3; i++) {
            ZZEditCellModel *model = temp[0][i];
        ...
        NSLog(@"%@\n%@\n%@",self.dataArray,self.headArray,self.titleArray);
        for (ZZEditCellModel *model in self.headArray) {
            NSLog(@"%@",model.title);
        }
    }
