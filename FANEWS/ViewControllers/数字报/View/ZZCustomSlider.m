...
- (NSDictionary*)textAttributesDict {
    if (!_textAttributesDict) {
        ...
        for (int i = 0; i < _partNameArray.count; i ++) {
            if (i == 0) {
                dict[NSForegroundColorAttributeName] = kColor(245, 108, 108); // 文字颜色
                ...
                dict[NSFontAttributeName] = [UIFont systemFontOfSize:14 + i * 2]; // 字体
                _textAttributesDict = dict;
            }
...
- (void)loadThumWithContext:(CGContextRef)context {
    ...
    if (!self.isFirst) {
        //滑块初始化
        self.thumbRect = [self.partRectArray[self.value] CGRectValue];
        self.isFirst = YES;
    }
