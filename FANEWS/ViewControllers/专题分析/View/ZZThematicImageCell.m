...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self) {
        ...
        [self.contentView addSubview:self.newPhotoImage];
        [self.contentView addSubview:self.lineLabel];
        
        [self.newPhotoImage mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        [self.titleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.mas_equalTo(self.contentView).offset(12);
            make.right.mas_equalTo(self.newPhotoImage.mas_left).offset(-15);
        ...
        [self.timeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.top.mas_equalTo(self.newPhotoImage.mas_bottom).offset(10);
        }];
...
+ (CGFloat)heightWithModel:(ZZTextModel *)model{
    ...
    return titleHeight + 55; // 动态高度 + 静态高度
}
...
- (void)initCellWith:(ZZTextModel *)model{
    ...
    [self.newPhotoImage sd_setImageWithURL:[NSURL URLWithString:@"https://alifei01.cfp.cn/creative/vcg/veer/1600water/veer-368621010.jpg"]];
}
- (UILabel *)titleLabel{
    if (!_titleLabel) {
...
- (UIImageView *)newPhotoImage{
    if (!_newPhotoImage) {
        _newPhotoImage = [[UIImageView alloc]init];
    }
    return _newPhotoImage;
}
